...
public final class EmployeeManagementController extends BaseServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // FIXME Step-4-1a: 社員情報管理サービスのインスタンスを生成しなさい。
        // Tips: 定義済みフィールド変数を使用
        // [ここへ記述]
        ...
        try {
            ...
            Logger.log(new Throwable(), "reqStatus  = " + this.responseBean.getRequestStatus());
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // リクエストよりID番号を取得（※削除時は複数の可能性あり）: 関数型インターフェース（ラムダ式）
        Function<HttpServletRequest, List<String>> rmdGetEmpIdList = (rmdRequest) -> {
            // FIXME Step-4-2a: 各jspよりPOSTで送信されたリクエストパラメーターのID番号を取得しなさい。
        ...
        try {
            ...
            // FIXME Step-4-3a: 社員情報管理サービスのインスタンス変数を生成しなさい。
            // Tips: 定義済みフィールド変数を使用
            ...
            // FIXME Step-4-4a: 取得結果（ResponseBean）をjspへ渡すための処理を記述しなさい。
    ...
    private RequestType getRequestType(final HttpServletRequest request) throws MVCException {
        ...
        try {
            if (Objects.isNull(requestTypeName)) {
                ...
                this.responseBean.setRequestStatus(1);
                this.responseBean.setEmployeeBeanList(new ArrayList<>(0));
            ...
            if (Objects.isNull(requestType) || this.responseBean.getRequestStatus() > 0) {
                // リクエスト対象の画面へ戻す
