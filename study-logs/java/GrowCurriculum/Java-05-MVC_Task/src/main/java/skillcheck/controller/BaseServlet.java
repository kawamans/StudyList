...
public abstract class BaseServlet extends HttpServlet {
    ...
    // FIXME Step-3a: 定数定義
    // FIXME Step-3-1a: リクエスト判別用のボタンの属性名を記述しなさい。
    ...
    // FIXME Step-3-2a: 実行結果表示用のjspファイルのパスを記述しなさい。
    ...
    protected boolean validateLogin(final HttpServletRequest request) throws IOException {
        ...
        try {
            // FIXME Step-3-3a: 社員情報管理サービスより、社員情報を取得する処理を呼び出しなさい。
            // Tips1: 社員情報管理サービスはインスタンスが生成済みのものを利用すること
        		// Tips2: 完全一致検索の社員情報取得を呼び出すこと
            // Tips3: メソッドに設定された引数の型を見ながら、どのような値を渡すべきか考えましょう
            // ←ここへ記述
        		EmployeeBean pEmployeeBean = new EmployeeBean(reqEmpId);
        		responseBean = ems.getEmployeeData(EmployeeDao.ExecuteCase.FIND_BY_EMPID, pEmployeeBean);

            // 最初の1件を取得
            resEmployeeBean = responseBean.getEmployeeBeanList().stream().findFirst().orElse(null);
    ...
    protected void executeSetExceptionInfo(final Exception e, final String message, final int listSize) {
        Logger.log(e);
        this.responseBean.setRequestStatus(2);
        this.responseBean.setMessage(message);
        if (listSize < 0) return;
        this.responseBean.setEmployeeBeanList(new ArrayList<>(listSize));
    }
